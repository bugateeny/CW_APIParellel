...
//Create JOB Callout (POST)
        String responseJC =
                given().log().all().header("Content-Type", "application/json")
                        .body(CallOut_Payload.jobCallOutAPI(siteAccount, description))  // With Dataprovider Dynamically control data  when payload is concatenated.  Objects in json can be changed here  or remove to use original payload
...
//UPDATE JOB Callout :                                                        http://192.168.132.176:54040/api.vixen/jobs/id/01M00095XXX   ID is updated by path param url
        given().log().all().header("Content-Type", "application/json")
                .body(CallOut_Payload.jobUpdateAPI("Bugatiny", "3rd Call out -  CCTV broken again ")) //  when payload is concatenated.  Objects in json can be changed here  or remove to use original payload
                .log().all()
...
//                .then().assertThat().statusCode(204);
    ...
    public Object[][] getData() {
        ...
        return new Object[][]{
                ...
                {"ADE004", "4nd CCTV call out"}};    //  Run multiple data sets @Test
    }
